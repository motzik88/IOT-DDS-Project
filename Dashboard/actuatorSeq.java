...
/**
* A sequence of actuator instances.
*/
public final class actuatorSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public actuatorSeq() {
        super(actuator.class);
    }

    public actuatorSeq (int initialMaximum) {
        super(actuator.class, initialMaximum);
    }

    public actuatorSeq (Collection elements) {
        super(actuator.class, elements);
    }

    public  actuator get(int index) { 
        return (actuator) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, actuator.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' actuator objects (beyond the original size of this object)
